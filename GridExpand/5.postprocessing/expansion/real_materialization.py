    ...
    for n150 in range(limit + 1):
        for n185 in range(limit + 1):
            for n240 in range(limit + 1):
                ...
                added_capacity = sum(count * item[1] for count, item in zip(counts, catalog))
                if added_capacity + 1e-12 < required:
                    continue
                total_duct_cost = sum(count * item[2] for count, item in zip(counts, catalog))
...
def _transformer_cost(required_kva: float, rated_kva: float, assumption: dict[str, Any]) -> tuple[float, str]:
...
def _critical_indices(db, run_id: int, stage: str) -> dict[tuple[str, int], int]:
    ...
    with db.engine.connect() as conn:
        ...
        return {(str(row["metric"]), int(row["asset_id"])): int(row["t_index"]) for row in rows}
...
def materialize_real_results(db, *, expansion_analysis_run_id: int, args) -> dict[str, int]:
    runs = _selected_runs(db, args)
    if runs.empty:
        raise RuntimeError("No real SWF power-flow summaries match the requested expansion scope.")
    ...
    for run in runs.to_dict("records"):
        ...
        if not source_file.exists():
            raise FileNotFoundError(f"Real SWF grid source does not exist: {source_file}")
        ...
        for cable in _cable_summaries(db, run_id, args.stage).to_dict("records"):
            ...
            if max_i_ka is None or max_i_ka <= 0 or installed_capacity_ka is None or loading_percent is None:
                raise ValueError(f"Real cable {lv_id}:{cable_id} lacks a finite installed capacity or P100 loading.")
            ...
            line_rows.append(
                {
                    **status_rows[-1],
                    ...
                    "geom_wkt": _line_wkt(net, from_bus, to_bus),
                    **costs,
                }
            )
        ...
        if rated_kva <= 0 or loading_percent is None:
            raise ValueError(f"Real grid LV {lv_id} lacks a finite transformer rating or P100 loading.")
        max_s_mva = loading_percent / 100.0 * rated_kva / 1000.0
        step = float(assumption["transformer_capacity_step_kva"])
        required_kva = max(rated_kva, math.ceil(max_s_mva * 1000.0 / step - 1e-12) * step)
        transformer_cost, transformer_basis = _transformer_cost(required_kva, rated_kva, assumption)
        equipment_name = None
    ...
    line_sql = text(
        ...
        INSERT INTO surrogrid.expansion_real_line_result (
            expansion_analysis_run_id, real_powerflow_run_id, real_grid_case_id,
            scenario_id, plz, lv_id, cable, cable_name, std_type, from_bus, to_bus,

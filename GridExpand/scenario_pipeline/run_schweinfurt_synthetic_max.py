#!/usr/bin/env python3
"""Run the Schweinfurt 2045 smoke run on one mixed-use synthetic grid.

Only the ``pre`` and ``post-hems-optimized`` model cases are produced. The
pipeline matches the Forchheim smoke runner (Step 2 demand allocation, Step 3
urbs optimization, Step 4 power flow) but targets a Schweinfurt mixed-use grid
from pylovo version 100.
"""
...
DEFAULT_RUN_CONFIG = (
    ...
    / "config"
    / "runs"
    / "schweinfurt_2045_synthetic.yaml"
)
DEFAULT_OUTPUT_DIR = GRIDEXPAND_DIR / "run_logs" / "schweinfurt_2045_synthetic_smoke_grid55"

# Mixed-use grid: 50 buildings = 27 Residential (118 households) + 18 Commercial + 5 Public.
SELECTED_GRID = {
    "ags": "9662000",
    "candidate_index": 9,
    "grid_result_id": 55,
    ...
    "bcid": 9,
    "n_buildings": 50,
    "n_residential_buildings": 27,
    "residential_share": 27 / 50,
    "residential_households": 118,
}
...
def main() -> int:
    ...
    if args.resume and manifest_path.exists():
        ...
        manifest: dict[str, Any] = {
            "run_id": "schweinfurt_2045_synthetic_smoke_grid55",
            "started_at": utc_now(),
            "finished_at": None,

...
# enforce that process capacity at site which outputs sellable == process capacity of process which takes as input buyable
# e.g. import capacity == feed_in capacity
...
def search_sell_buy_tuple(m, stf, sit_in, pro_in, coin):
    ...
    # buy_out = (stf, site, output_commodity) = all output processes at site which are the same process as the regarded input process
    buy_out = set([(x[0], x[1], x[3])
                   for x in pro_output_tuples
                   if x[2] == pro_in])
    # search the sell process for the output_commodity from the buy process
    # all output processes at site which have as output commodity the input commodity of the regarded input process + sell it!
    sell_output_tuple = ([x
                          for x in pro_output_tuples
                          if x[3] in m.com_sell])
    
    for k in range(len(sell_output_tuple)):
        sell_pro = sell_output_tuple[k][2]   # loop over over all 

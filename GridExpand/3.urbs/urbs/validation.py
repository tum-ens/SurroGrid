...
def validate_input(data):
    ...
    #     if not (len(data['type_period'].dropna(axis=0, how='all').index) % 24 == 0 or
    #             len(data['type_period'].dropna(axis=0, how='all').index) % 24 - 1 == 0):
    ...
    #     if min(data['type_period'].iloc[1:,0]) < 1:
    #         print('Warning: weighting_typeperiod < 1')

    #     if sum(data['type_period'].loc[:,'weight_typeperiod'].dropna(axis=0, how='all')) != 8760:

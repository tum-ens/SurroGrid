...
def create_model(data, global_settings):
    ...
    Args:
        ...
        - objective: Either "cost" or "CO2" for choice of objective function,
          default: "cost"
        - hoursPerPeriod: number of tsam periods
        - weighting_order: weights of each tsam period
        - assumelowq: high R to Q ratio present
        - dual: set True to add dual variables to model output
          (marginally slower), default: True
        - grid_plan_model: ???
        - bui_react_model: Is this run as urbs-bui-react
        - flexible_heat: ???
    ...
    ### Setup Model Basics: 
    m = pyomo_model_prep(data, global_settings["timesteps"])  # preparing pyomo model
    # m.name = 'urbs'
    # m.created = datetime.now().strftime('%Y%m%dT%H%M')
    # m._data = data
    # m.grid_plan_model = False
...
def pyomo_assign_params(m, global_settings):
    ...
    # m.obj = pyomo.Param(
    #     initialize=global_settings["objective"],
    #     within=pyomo.Any,
...
def pyomo_assign_basic_sets(m, global_settings):
    ### Extract relevant global settings:
    ...
    #     within=m.stf * m.sit,
    #     initialize=tuple(m.site_dict["Building"].keys()),
    ...
    #     doc='Processes producing elec-react at site, e.g. (2020, node1, PV_private, Elec-Reactive)')
...
def pyomo_assign_basic_variables(m, global_settings):
    ...
    ### Commodities (two index lists act as multiindex)
...
def pyomo_assign_basic_constraints(m, global_settings):
    ...
    m.def_process_input = pyomo.Constraint(
        m.tm, m.pro_input_tuples 
            #- m.pro_partial_input_tuples - m.pro_on_off_input_tuples - m.pro_partial_on_off_input_tuples,
        ,
        rule=def_process_input_rule,
        doc='process input = process throughput * input ratio')
    m.def_process_output = pyomo.Constraint(
        m.tm, m.pro_output_tuples 
            # - m.pro_partial_output_tuples - m.pro_on_off_output_tuples - m.pro_partial_on_off_output_tuples -
    ...
    # This is dangerous as expansion rule bounds apply to newly installed capacity wheras actually they are meant for total capacity in input 
...
def res_vertex_rule(m, tm, stf, sit, com, com_type):
    ...
    # SupIm commodities are converted by process to pro_out commodity (which is accounted for here!)
    # if com in m.com_env:
    #     return pyomo.Constraint.Skip
    if (com in m.com_supim) or com=="electricity_hp": return pyomo.Constraint.Skip

    # Storage, transmission, process production/consumption 
    # com_bal = (pro_in+sto_in+tra_out)-(pro_out+sto_out+tra_in)
    power_surplus = - commodity_balance(m, tm, stf, sit, com)

    # surplus += taking from stock 
    ...
    # surplus + buy - sell
    if m.mode['bsp']: power_surplus += bsp_surplus(m, tm, stf, sit, com, com_type)

    # surplus - demand
    if com in m.com_demand:
        try: power_surplus -= m.demand_dict[(sit, com)][(stf, tm)]
...
def res_stock_total_rule(m, stf, sit, com, com_type):
    if (com not in m.com_stock) and (com not in m.com_demand):
        ...
        total_consumption = 0
        for tm in m.tm:
            total_consumption += (m.e_co_stock[tm, stf, sit, com, com_type] * m.typeperiod['weight_typeperiod'][(stf, tm)])
...
def def_costs_rule(m, cost_type):
    ...
    if cost_type == 'Invest':
        ...
        # if m.mode['tra']:
        #     cost += transmission_cost(m, cost_type)
        if m.mode['sto']: cost += storage_cost(m, cost_type)
        return m.costs[cost_type] == cost
    ...
    elif cost_type == 'Revenue': return m.costs[cost_type] == revenue_costs(m)
    elif cost_type == 'Purchase': return m.costs[cost_type] == purchase_costs(m)
    elif cost_type == 'Power price':  #LVDS # IMPLEMENT: this might not be weighted properly

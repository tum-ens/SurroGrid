...
def get_entity(instance, name):
    ...
    Returns:
        a Pandas DataFrame with domain (e.g. [stf, pro, cap]) as index and values (or 1's, for sets) of
        entity name. For constraints, it retrieves the dual values
    """

    # Should there already be a _result attribute, then return that (should not be the case)
    ...
    # There shouldn't be any exception as the onset names were extracted by definition as attributes
    try:
    ...
    ########## extract values
    # takes set.data() which is list of set members (e.g. tuples (stf, sit, pro) for multidimensional or just com (not tuple))
    # to dataframe with index as set member name (as multicolumn for multidimensional, a one column for one-dimensional) with values all 1
    # has no column names yet
    ...
    if isinstance(entity, pyomo.Set):
        ...
        # hence, make column index equal to entity name and append underscore
        if not labels:
            labels = [name]
            name = name + '_'
    ...
    # dt, obj, weight
        ...
        if entity.dim() > 1:
            ...
            # dt -> "None" 1, obj -> "None" cost, weight -> "None" 1
            results = pd.DataFrame(
                [(v[0], v[1].value) for v in entity.items()])
            labels = ['None']

    # cap_pro, cap_sto_c, cap_sto_p
        ...
        if entity.dim() > 1:
            ...
            results = pd.DataFrame(
                [(v[0], v[1]()) for v in entity.items()])
        # cap_pro -> stf, sit, pro, capacity; cap_sto_c/p -> stf, sit, sto, com, cap
        else:
            results = pd.DataFrame(
                [(v[0], v[1]()) for v in entity.items()])
            labels = ['None']
...
def list_entities(instance, entity_type):
    ...
    def filter_by_type(entity, entity_type):
        if entity_type == 'set':
            return isinstance(entity, pyomo.Set) and not entity.is_constructed()

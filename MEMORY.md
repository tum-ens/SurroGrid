...
## 2026-06-03 - DB-backed power-flow max-loading plotting

- What was decided: Extend `GridExpand/4.powerflow/plotting/powerflow_plotting.py` to read DB-backed Step 4 results, compute line loading from stored `i_from_ka` values against the original pandapower `net.line.max_i_ka`, and produce a pandas-ready tabular summary and Plotly comparison for pre/status-quo versus post/full-pipeline loading, with the table integrated into the plotting notebook instead of generating CSV artifacts.
...
## 2026-06-05 - Operational min-temperature timeslice for small AGS

- What was decided: Supersede the earlier timeslice-revert decision for the Munich scaling problem and make the requested one-week timeframe path operational. Step 2 now supports `min_temperature_week`, `max_solar_radiation_week`, and `max_base_electricity_demand_week` as 168-hour stress-screening modes with explicit metadata. Non-full-year HDF5 outputs get a timeframe suffix, Step 3 can select exact `.h5` filenames and derives the modeled horizon from input length, Step 4 writes DB results with timestamps from timeframe metadata, and the AGS runner passes exact suffixed filenames through Steps 2-4.
- Why: Full Munich AGS runs remain too slow, and the TASK.md goal requires a small-grid timeslice to run through Step 2, Step 3, Step 4, and AGS-runner validation while keeping default `full_year` behavior unchanged.
- What was rejected and why: Reintroducing separate scenario columns was rejected because `surrogrid.scenario.assumptions` stores the timeframe metadata without schema churn. Supporting DST-transition weeks was rejected for this slice because the existing DST helpers are full-year index based; selected weeks that cross fixed DST transition hours now fail explicitly. Running expensive emobpy generation in timeslice mode was rejected; timeslice modes require `--mobility-source pool`.
- Verification: `uv run --project .. python main.py 09278140 --storage db --profiles all --mobility-source pool --timeseries-storage temp --timeframe-mode min_temperature_week --n_cpu 1` produced `9278140-00_94342_1_-1_min_temperature_week.h5` with 168 rows in `urbs_in` time-series and selected 2009-01-22T12:00:00+00:00 to 2009-01-29T11:00:00+00:00. Step 3 solved the exact suffixed file with 168 modeled hours. Step 4 wrote DB pre/post results with `max_t=167`. The AGS runner passed end to end for `09278140 --limit 1` and validated the 168-hour DB horizon.

## 2026-06-05 - Munich min-temperature timeslice completed

- What was decided: Run all 298 AGS 09162000 grid candidates with `min_temperature_week`, temporary Step 2 time-series storage, 3 concurrent grid workers, Step 2/4 at 4 CPUs per worker, and dynamic Step 3 up to 24 clusters with `URBS_CLUSTER_CONCURRENCY=8`.
- Why: The 168-hour timeslice makes full Munich feasible for screening, RAM was no longer the Step 3 bottleneck, and the 3-worker runner avoided the earlier DB startup deadlocks while keeping the machine busy. Run-level timeframe assumptions and schema readiness checks keep concurrent candidates from corrupting or racing metadata.
- What was rejected and why: Raising grid workers back to 4 was rejected because it had already reproduced DB startup deadlocks. Reducing Step 3 cluster concurrency was rejected because the 8-concurrent setting completed even 24-cluster URBS jobs quickly without memory pressure. Treating transient Open-Meteo failures as deterministic grid failures was rejected because the final failed-only rerun passed them.
- Verification: `/tmp/surrogrid_munich_timeslice_pilot` completed with all 298 candidates `done`. The main resumed pass finished 245/249 selected candidates and isolated 4 failures; the failed-only rerun passed candidates 48, 83, 231, and 255. Candidate 83 passing verified the DistrictGenerator nearest-ZIP fallback for missing ZIP 81248. `git diff --check` and py-compile checks for the touched Step 2, Step 3/runner, Step 4, database, and timeframe modules passed.

## 2026-06-05 - Timeslice transformer plots use relative timestep axis

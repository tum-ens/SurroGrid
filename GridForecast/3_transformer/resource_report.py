"""Lightweight CPU/time/memory reporting utilities.

`resource_report()` is used to wrap long-running steps (e.g. training or
evaluation) and prints:
    - wallclock duration
    - CPU user/system times (optionally including child processes)
    - peak resident memory (RSS), when the platform exposes it

The implementation uses a best-effort strategy across platforms:
    - POSIX: `resource.getrusage`
...
def _sec_to_hms(sec: float) -> str:
    """Format seconds as `Hh MMm SS.SSs`."""
    h = int(sec // 3600)
    m = int((sec % 3600) // 60)
    s = sec % 60
